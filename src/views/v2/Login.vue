<template>
  <div
    class="index"
    :style="bgImg ? { 'background-image': 'url(' + bgImg + ')' }:{backgroundColor:'#fff'}"
  >
      ...
      <van-form>
        <van-cell-group class="cell-style">
          <van-field
            ...
            clearable
            placeholder="输入手机号码"
            :left-icon="version == 'v2' ? tel_url2 : tel_url1"
            type="digit"
            maxlength="11"
            v-reset-page
          />
          <van-field
            ...
            center
            clearable
            :left-icon="version == 'v2' ? code_url2 : code_url1"
            placeholder="输入短信验证码"
            maxlength="6"
            v-reset-page
          >
            <template #button>
              <van-button
                ...
                hairline
                round
                :class="version == 'v2' ? 'text-red-color':'text-blue-color'"
                :disabled="disabled"
                @click="getCode"
              >
                ...
                >
                  <template v-slot="timeData">
                    <span style="color:#fff">{{ timeData.seconds }}秒后重发</span>
                  </template>
                </van-count-down>
              </van-button>
            </template>
        ...
        <van-button
          ...
          @click="onSubmit"
          :loading="loading"
        >{{version == 'v2' ?'登录':'领取兑换码'}}</van-button>
      </van-form>
...
export default {
  ...
  data() {
    return {
      ...
      bgImg: sessionStorage.getItem("bgImg"),
  ...
  methods: {
    ...
    getCode() {
      ...
      if (phone == "") {
        ...
        let myData = { activity_id: this.activeId, mobile: this.phone };
        myData = aes_encryptObject(myData);
        this.$http
          .post("/comms/send-sms", myData)
          .then(res => {
            if (res.data.code == 0) {
              this.disabled = true;
    ...
    onSubmit() {
      ...
      let phone = this.phone;
      let apiUrl = this.version == "v2" ? "/v2/users/login" : "/users/login";
