<template>
  <div class="result">
    <div class="img_text">
      <img v-if="(rq.return_code == 0) || (rq.return_code == 220015)" :src="url2" alt />
      <img v-else :src="url1" alt />
      <div>
        <span v-if="(rq.return_code == 0) || (rq.return_code == 220015)" style="color:#48CB20">下单成功</span>
    ...
    <div class="field-box">
      ...
      <div class="pay_msg" v-if="rq.return_code != 0">
        <p>温馨提示</p>
    ...
    <van-button
      v-if="(rq.return_code!=0) && (rq.return_code!=220015)"
      type="danger"
      round
      @click="payAgain"
      block
      class="text-color"
    >重新支付</van-button>
    <van-button type="default" round block @click="goIndex">返回首页</van-button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    payAgain() {
      if(this.rq.order_type == 1) this.repayment();
      else this.payment();
    },
    payment() {
      //套餐支付
      let myData = aes_encryptObject({
        package_id: this.rq.product_id,
        activity_id: this.rq.activity_id
      });
      this.$http
    ...
    repayment() {
      ...
      let mydata = aes_encryptObject({
        activity_id: this.rq.activity_id,
        out_trade_no: this.rq.out_trade_no,
        order_type: this.rq.order_type
      });
      this.$http
        .post("/v2/payments/payment-activity", mydata)
        .then(res => {
          if (res.data.code == "0") {
